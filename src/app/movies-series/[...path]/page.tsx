...
// Skip routes whose article is missing or still a draft, so a scaffolded guide can sit in the repo safely.
export function generateStaticParams(){return Object.entries(routes).filter(([slug])=>getArticleBySlug(slug)).map(([,path])=>({path:path.split("/")}));}
