  ...
  return (
    <>
      ...
      <header className="bg-paper font-mono w-full relative z-50">
        {/* ═══ TIER 1: TOP BAR ═══ */}
        <div className="border-b border-grey-line lw-anim-topbar">
          {/* Desktop */}
          <div className="hidden md:block px-7 py-2.5">
            <div className="grid grid-cols-[1fr_auto_1fr] items-center max-w-6xl mx-auto">
              ...
              <div className="flex items-center gap-0">
                {TOP_BAR_LEFT.map((item, i) => (
                  <span key={item.label} className="inline-flex items-center">
                    {i > 0 && (
                      <span className="text-grey-line text-xs select-none mx-1">
                        |
                      </span>
                    )}
                    <Link
                      href={item.href}
                      className="text-ink no-underline px-2.5 py-0.5 text-[10.5px] font-bold tracking-widest uppercase whitespace-nowrap hover:text-grey-text transition-colors duration-200"
                    >
                      {item.label}
                    </Link>
                  </span>
              ...
              <div className="flex items-center">
                ...
                <span className="font-serif text-xl font-medium italic tracking-tight normal-case px-3 select-none">
                  Wall&Fifth
                </span>
                <span className="text-grey-line text-xs select-none mx-2">|</span>
              </div>
              ...
              <div className="flex items-center justify-end gap-0">
                ...
                <Link
                  href="/subscribe"
                  className="text-ink px-2.5 py-0.5 text-[10.5px] font-bold tracking-widest uppercase whitespace-nowrap underline underline-offset-2 hover:text-grey-text transition-colors duration-200"
                >
                  Subscribe
                </Link>
                <span className="text-grey-line text-xs select-none mx-1">|</span>
          ...
          <div className="flex md:hidden items-center justify-between px-4 py-2.5">
            <button
              ...
              className="bg-transparent border-none cursor-pointer p-1 text-ink"
            ...
            <span className="font-serif text-base font-medium italic tracking-tight select-none">
              Wall&Fifth
            </span>
            <button
        ...
        <div className="max-w-5xl mx-auto px-6 pt-8 pb-7 md:pt-10 md:pb-8">
          ...
          <div className="hidden lg:grid grid-cols-[1fr_auto_1fr] items-center gap-6">
            <div className="flex flex-col gap-1 items-start lw-anim-trending-left">
              {TRENDING_LEFT.map((item) => (
                <Link
                  key={item.title}
                  href={item.href}
                  className="lw-trending-link font-serif text-sm italic text-grey-dark no-underline leading-relaxed hover:text-ink transition-colors duration-200"
                >
                  {item.title}
                </Link>
              ))}
            ...
            <div className="text-center relative">
              <button
                onClick={() => setMenuOpen(true)}
                className="absolute -left-16 top-1/2 -translate-y-1/2 bg-transparent border-none cursor-pointer p-2 text-ink hover:scale-110 transition-transform duration-200"
                aria-label="Open menu"
              >
                <svg
                  width="24"
              ...
              <div className="lw-anim-logo">
                ...
                <Link
                  href="/"
                  className="font-serif text-7xl font-extrabold leading-none -tracking-[0.03em] text-ink no-underline block"
                >
                  LocoWeekend
                </Link>
              </div>

              <p className="font-mono text-[15px] text-grey-text mt-3.5 tracking-wide lw-anim-tagline">
                We used to write about tourism, now we write about everything.
              </p>

              <button
                onClick={() => setSearchOpen(!searchOpen)}
                className="absolute -right-16 top-1/2 -translate-y-1/2 bg-transparent border-none cursor-pointer p-2 text-ink hover:scale-110 transition-transform duration-200"
                aria-label="Search"
              >
                <svg
                  width="22"
            ...
            <div className="flex flex-col gap-1 items-end lw-anim-trending-right">
          ...
          <div className="lg:hidden text-center">
            <div className="lw-anim-logo">
              ...
              <Link
                href="/"
                className="font-serif text-5xl sm:text-6xl font-extrabold leading-none -tracking-[0.03em] text-ink no-underline block"
              >
                LocoWeekend
              </Link>
            </div>
            <p className="font-mono text-[13px] text-grey-text mt-2.5 tracking-wide lw-anim-tagline">
              We used to write about tourism, now we write about everything.
            </p>
          </div>
        </div>
        ...
        <div className="relative">
          <div className="h-[2.5px] bg-ink lw-anim-border" />
          <div className="absolute left-6 sm:left-8 top-0 -translate-y-full bg-yellow text-ink font-mono text-[11px] font-bold tracking-[0.14em] uppercase px-3.5 py-1.5 leading-none lw-anim-edit-tag">
            THE EDIT
          </div>
          <nav className="flex items-center justify-center py-3.5 px-4 sm:px-6 max-w-5xl mx-auto overflow-x-auto lw-anim-catnav">
            {CATEGORY_NAV.map((item, i) => (
              <span key={item.label} className="inline-flex items-center shrink-0">
                {i > 0 && (
                  <div
                    className="w-px h-4 bg-grey-text/35 shrink-0"
                    aria-hidden="true"
                  />
                )}
                <Link
                  href={item.href}
                  className="lw-cat-link font-mono text-sm font-bold tracking-wider uppercase text-ink no-underline px-4 sm:px-5 py-1 whitespace-nowrap hover:text-grey-text transition-colors duration-200"
                >
                  {item.label}
                </Link>
              </span>
      ...
      >
        ...
        {CATEGORY_NAV.map((item, i) => (
          <Link
            ...
            href={item.href}
            onClick={() => setMenuOpen(false)}
            className="lw-menu-item font-mono text-base font-bold tracking-[0.12em] uppercase text-paper no-underline py-3 px-6 hover:text-yellow transition-colors duration-200"
        ...
        <div
          className="lw-menu-item mt-8 w-8 h-0.5 bg-yellow"
          style={{ animationDelay: "0.4s" }}
        />

        <div
          className="lw-menu-item flex items-center gap-6 mt-4"
          style={{ animationDelay: "0.45s" }}
        >
          <Link
            href="/subscribe"
            onClick={() => setMenuOpen(false)}
            className="font-mono text-xs text-paper/50 no-underline tracking-widest uppercase hover:text-yellow transition-colors duration-200"
          >
            SUBSCRIBE
          </Link>
          <Link
